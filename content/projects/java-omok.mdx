...
-   **커뮤니티 및 편의 기능**:
    ...
    -   **사용자 프로필**: 회원가입 시 `thumbnailator` 라이브러리와 직접 구현한 `CropImageWindow`를 통해 프로필 이미지를 편집/저장하고, 이 이미지가 오목돌로 사용됩니다.
...
-   **`Java Socket (TCP/IP)`**: 서버의 핵심 통신 기술로, 클라이언트의 연결 요청을 받고 데이터를 주고받는 역할을 합니다.
-   **`Multi-threaded Server`**: `ChatServer`가 클라이언트 접속 시마다 `ClientHandler` 스레드를 생성하는 **1-Client-1-Thread** 모델을 적용했습니다. 이를 통해 여러 클라이언트의 동시 요청을 안정적으로 처리할 수 있습니다.
-   **`Custom Protocol`**: "CRTROOM", "PRIVCHAT", "GAME:MOVE" 등과 같이 `|` 또는 공백으로 구분된 문자열 기반의 자체 프로토콜을 설계하여 클라이언트와 서버 간의 다양한 요청과 응답을 체계적으로 관리했습니다.

#### 2. 프론트엔드 (Frontend)

-   **`Java Swing`**: 모든 사용자 인터페이스를 Swing으로 구축했습니다. `JFrame`, `JPanel`, `JTable` 등 기본 컴포넌트뿐만 아니라, `paintComponent` 메소드를 오버라이드하여 UI를 직접 렌더링하는 다수의 커스텀 컴포넌트를 구현했습니다.
    -   **`BoardLabel`**: 오목판의 격자와 격자점, 사용자의 프로필 이미지를 이용한 오목돌, 마우스 오버 시 미리보기 등을 직접 그려주는 핵심 게임 UI 컴포넌트입니다.
    -   **`ChatBubblePanel`**: 메시지 길이와 이모티콘 유무에 따라 동적으로 크기가 변하는 말풍선 UI를 구현했습니다.
    -   **`CustomScrollBarUI`**: 채팅창의 스크롤바 디자인을 커스터마이징하여 미적 완성도를 높였습니다.
    -   **`FontLoader`**: `.ttf` 폰트 파일을 동적으로 로드하여 사용자 PC에 폰트가 설치되지 않아도 일관된 디자인을 제공하도록 했습니다.

#### 3. 데이터베이스 (Database) & Persistence

-   **`MySQL` & `JDBC`**: 사용자 정보, 게임 전적, 채팅 로그 등 모든 데이터를 MySQL에 저장하고, JDBC API를 통해 Java 애플리케이션과 연동했습니다.
-   **`DAO (Data Access Object) Pattern`**: DB 연결을 담당하는 `DBmanager`, 유저 정보 관련 SQL을 모아둔 `UserInfoDb`, 채팅 메시지 관련 SQL을 모아둔 `MessageDbManager`로 역할을 분리하여 데이터 접근 로직을 캡슐화하고 코드의 유지보수성을 향상시켰습니다.
-   **`Soft-delete`**: 관리자가 사용자나 채팅 메시지를 삭제할 때, 실제 DB에서 제거하는 대신 `_bin` 접미사가 붙은 백업 테이블로 데이터를 옮기도록 설계했습니다. 이를 통해 데이터의 실수 없는 관리와 복원 기능을 구현했습니다.

#### 4. 외부 API 및 라이브러리

-   **`Public Data Portal API`**: 기상청의 초단기예보 API를 HTTP로 호출하여 날씨 정보를 가져왔습니다.
-   **`NaverMail API (javax.mail)`**: 비밀번호 찾기 기능 구현을 위해 Naver의 SMTP 서버를 통해 사용자에게 이메일을 발송하는 `NaverMailSender`를 구현했습니다.
-   **`Thumbnailator`**: 프로필 이미지 업로드 시, 사용자가 원하는 영역을 정사각형으로 자르고 리사이징하는 데 사용된 이미지 처리 라이브러리입니다.
...
#### 3. 커스텀 프로토콜 처리 (`ClientHandler.java`)
클라이언트로부터 받은 문자열 메시지를 `startsWith` 메소드로 파싱하여, 약속된 동작을 수행하는 부분입니다. 이를 통해 채팅, 유저 정보 요청, 게임방 관리 등 다양한 기능을 구분하여 처리합니다.
```java
// in ChatServer.java > ClientHandler.run()
while ((message = in.readLine()) != null) {
    if (message.startsWith("GET_USER_INFO")) {
        ...
        // 게임방 생성 로직
    } else if (message.startsWith("ENTROOM")) {
        // 게임방 입장 로직
...
#### 5. SMTP를 이용한 이메일 전송 (`NaverMailSender.java`)
