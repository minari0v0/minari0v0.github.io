...
'텅장수강러'는 대학생들의 수강신청과 학교생활을 돕기 위해 개발된 풀스택 웹 애플리케이션으로, <br>
이 프로젝트는 강의 평가, 개인 시간표 관리, 학생 커뮤니티, 캠퍼스 생활 정보(날씨, 학식, 학사 일정) 등 대학생에게 필수적인 다양한 기능을 제공합니다.
## 프로젝트 구조
```
.
├── pom.xml
...
- **Apache Tomcat 9.0**: 서블릿 컨테이너 (웹 서버)
...
-- 신고 내역
reports (id, content_type, content_id, reporter_id, reason, status, admin_id, admin_action)

-- 사용자 등급 업데이트 프로시저
PROCEDURE UpdateUserGrade(IN p_user_id INT)
```
...
'텅장수강러' 프로젝트는 웹 개발의 각 계층을 깊이 있게 이해하고, 실제 사용자의 필요를 반영한 기능을 구현하며, 다양한 프론트엔드 상호작용과 백엔드 데이터 처리를 통합하는 귀중한 경험을 제공했습니다. 특히, 프레임워크에 의존하기보다 순수 Java와 웹 표준 기술 스택으로 직접 시스템을 구축하며 웹 개발의 근간 원리를 체득할 수 있었던 점이 가장 큰 성과라고 생각합니다. 이 프로젝트를 통해 얻은 경험과 지식은 향후 더 복잡하고 확장 가능한 웹 서비스를 개발하는 데 좋은 기반이 될 것입니다.
