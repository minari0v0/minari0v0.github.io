...
#### 3. 통계와 AI를 결합한 하이브리드 무게 예측
단일 이미지 분석의 한계를 극복하기 위해 통계 데이터와 생성형 AI의 장점을 결합했습니다. 자체 구축한 표준 무게 DB를 우선적으로 적용하되, 이미지 내 객체 크기 비율을 반영하여 오차를 줄였습니다. DB 미등록 물품의 경우 Gemini API가 예측하도록 설계했으나, 결과값이 비정상적인 범위를 벗어나지 않도록 **Min-Max 클리핑(Clipping) 로직**을 적용하여 데이터의 신뢰성을 확보했습니다.
```python
...
- **추론 성능 최적화**: TensorRT 가속화 및 하이브리드 파이프라인 설계를 통해 정확도를 유지하면서 전체 추론 속도 약 35% 향상
...
또한, 이번 프로젝트에서는 TensorRT의 기본 변환(FP16)을 사용했으나, 향후 QAT(Quantization Aware Training, 양자화 인식 학습) 기법을 도입하여 모델을 재학습시킨다면, YOLO 모델의 정확도 손실을 최소화하면서도 Int8 정밀도의 극한의 추론 속도를 확보하는 것이 가능할 것으로 보입니다.
